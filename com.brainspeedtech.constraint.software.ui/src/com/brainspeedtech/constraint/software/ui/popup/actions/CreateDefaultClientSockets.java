...
public class CreateDefaultClientSockets implements IObjectActionDelegate {
	 ...
	 */
	public CreateDefaultClientSockets() {
		super();
	}
...
//		if (domain == null) return;
		...
		for(Resource res: domain.getResourceSet().getResources()) {
			Layer localLayer = (Layer)EcoreUtil.getObjectByType(res.getContents(), LayerPackage.eINSTANCE.getLayer());
			if (localLayer == null) continue;
			Collection<Object> sockets = EcoreUtil.getObjectsByType(localLayer.eContents(), SoftwarePackage.eINSTANCE.getTCPSocketTube());
			for(Object sObj: sockets) {
				if (!(sObj instanceof TCPSocketTube)) continue;
		...
		for(TubeInstance tInst: instance.getTube()) {
			...
			socketTube.setBlurb("TCP Client Socket for " + tInst.getName());
