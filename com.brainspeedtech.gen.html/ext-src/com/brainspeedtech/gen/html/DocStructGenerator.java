...
public class DocStructGenerator {
	...
	public static void generate(SwarmCachingEditingDomain domain,
		...
		for (final Document struct : structs) {
			...
			String name = struct.eResource().getURI().lastSegment()
					.replaceAll("\\.struct$", "");
			new GeneratorFile(genPrefix + name + ".html") {
				@Override
				public String getFileContents() {
					return new HTMLStructFile().generate(struct);
				}
			};
			
			new GeneratorFile(genPrefix + name + ".tex") {
				@Override
				public String getFileContents() {
					return new TexStructFile().generate(struct);
				}
			};
