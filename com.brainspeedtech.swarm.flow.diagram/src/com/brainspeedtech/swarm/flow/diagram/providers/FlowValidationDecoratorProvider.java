...
import com.brainspeedtech.swarm.flow.diagram.part.FlowDiagramEditor;
 ...
 * @generated
 */
public class FlowValidationDecoratorProvider extends AbstractProvider implements
		IDecoratorProvider {
	 ...
	 * @generated
	 */
	private static final String MARKER_TYPE = FlowDiagramEditorPlugin.ID
	...
	public void createDecorators(IDecoratorTarget decoratorTarget) {
		...
		if (editPart instanceof GraphicalEditPart
			...
			if (((DiagramEditDomain) ed).getEditorPart() instanceof FlowDiagramEditor) {
	...
	public boolean provides(IOperation operation) {
		...
		return view != null
				&& FlowEditPart.MODEL_ID.equals(FlowVisualIDRegistry
						.getModelID(view));
	}
	...
	public static class StatusDecorator extends AbstractDecorator {
		...
		public void refresh() {
			...
			EObject eObject = (EObject) getDecoratorTarget().getAdapter(
					EObject.class);
			if (eObject != null) {
				XMLResource res = (XMLResource) eObject.eResource();
				elementId = res.getID(eObject);
			}
			if (elementId == null) {
				return;
			}
	...
	static class MarkerObserver implements IFileObserver {
		...
		private String getView(String objectid) {
			...
			while (iterator.hasNext()) {
				...
				if (object instanceof Shape) {
					...
					if(shape.getElement() == null || resource == null || resource.getID(shape.getElement()) == null) continue;
					if (resource.getID(shape.getElement()).equals(objectid)) {
						return resource.getID(shape);
					}
				}
		...
		public void handleMarkerDeleted(IMarker marker, Map attributes) {
			...
			refreshDecorators(getView(viewId), diagram);
		}
